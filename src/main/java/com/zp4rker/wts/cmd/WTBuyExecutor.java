...
public class WTBuyExecutor implements CommandExecutor {

	public WTBuyExecutor() {
		Help.commands.add(ChatColor.translateAlternateColorCodes('§', "§e/wheretobuy <item>"));
	}

	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (cmd.getLabel().equalsIgnoreCase("wheretobuy")) {
			if (sender instanceof Player) {
				if (sender.hasPermission("wheretoshop.buy")) {
					if (args.length == 1) {
						for (Config config : WhereToShop.shops.values()) {
							if (config.get(args[0]) != null) {
								if (config.getBoolean(args[0] + ".buy", false)) {
						...
						sender.sendMessage(WhereToShop.NOBUY);
					} else {
